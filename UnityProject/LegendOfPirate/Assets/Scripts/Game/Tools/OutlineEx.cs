...
namespace MBU3000
{
    ...
    public class OutlineEx : BaseMeshEffect
    {
        ...
        private static List<UIVertex> m_VetexList = new List<UIVertex>();//避免多次创建
        ...
        protected override void Awake()
        {
            ...
            var shader = Shader.Find("Custom/OutlineEx");
            base.graphic.material = new Material(shader);

            //UIVertex的非必须成员的数据默认不会被传递进Shader。所以我们需要修改UI组件的Canvas的additionalShaderChannels属性，让tangent成员也传入Shader。
        ...
        private void _ProcessVertices()
        {
            for (int i = 0, count = m_VetexList.Count - 3; i <= count; i += 3)
            {
                    ...
                    > Mathf.Abs(Vector2.Dot((pos3 - pos2).normalized, Vector2.right)))
                {
                    triX = pos2 - pos1;
                    triY = pos3 - pos2;
                    uvX = v2.uv0 - v1.uv0;//uv宽
                    uvY = v3.uv0 - v2.uv0;//uv高
                }
                else
                {
                    triX = pos3 - pos2;
