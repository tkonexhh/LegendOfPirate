using System.Collections;
using System.Collections.Generic;
using UnityEngine;


namespace GameWish.Game
{
	public class LanguageKeyDefine 
	{
		public const string TestKey = "";
        #region Common
        public const string FIXED_TITLE_LV_Ⅰ = "LV:{0}";
        public const string FIXED_TITLE_LV_Ⅱ = "LV{0}";
        public const string Fixed_Title_Upgrade = "Fixed_Title_Upgrade";
        public const string Fixed_Title_Train= "Fixed_Title_Train";
        public const string Fixed_Title_Auto= "Fixed_Title_Auto";

        public const string COMMON_SAW_ADV= "看了一个广告";
        public const string COMMON_CONT_Ⅰ= "There are no idle roles";
        public const string COMMON_CONT_Ⅱ= "The level has reached the maximum";

        public const string MONTH_JANUARY = "January";
        public const string MONTH_FEBRUSRY = "February";
        public const string MONTH_MARCH = "March";
        public const string MONTH_APRIL = "April";
        public const string MONTH_MAY = "May";
        public const string MONTH_JUNE = "June";
        public const string MONTH_JULY = "July";
        public const string MONTH_AUGUST = "August";
        public const string MONTH_SEPTEMBER = "September";
        public const string MONTH_OCTOBER = "October";
        public const string MONTH_NOVEMBER = "November";
        public const string MONTH_DECEMBER = "December";
        #endregion

        #region ChargingInterfacePanel
        public const string CHARGINGINTERFACE_PURCHASED = "Purchased";
        public const string CHARGINGINTERFACE_SUCCESSFULPURCHASED = "Successful Purchase";
        #endregion

        #region TrainingRoom
        public const string Fixed_Title_TrainingRoom = "Fixed_Title_TrainingRoom";
        public const string TRAININGROOM_CONT_Ⅰ = "There is no free space";
        public const string TRAININGROOM_CONT_Ⅱ = "Please select training role";
        public const string TRAININGROOM_CONT_Ⅲ = "There is no free role";
        #endregion

        #region LibraryRoom
        public const string LIBRARYROOM_CONT_Ⅰ = "There is no free space";
        public const string LIBRARYROOM_CONT_Ⅱ = "Please select libraryRoom role";
        public const string LIBRARYROOM_CONT_Ⅲ = "There is no free role";
        #endregion

        #region InternalPurchase
        public const string INTERNALPURCHASE_PROFITTITLE = "profit:{0}%";
        public const string INTERNALPURCHASE_PRICE = "Only ${0} / 30 days";
        public const string INTERNALPURCHASE_DUEDATE = "Due on {0} lafter purchase";
        public const string INTERNALPURCHASE_DAILY_COUNTDOWN = "Refresh {0}";
        #endregion

        #region Black Market
        public const string BLACKMARKET_REFRESHCOUMT_OVER = "The number of refreshes has been used up";
        #endregion
    }
}