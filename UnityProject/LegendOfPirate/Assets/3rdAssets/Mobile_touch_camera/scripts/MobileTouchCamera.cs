...
#pragma warning restore 414
        ...
        private Action<RaycastHit> m_OnPickItemAction;
        private Action<RaycastHit2D> m_OnPickItem2DAction;

        public void RegisterPickItemEvent(Action<RaycastHit> ac)
        {
            if (m_OnPickItemAction != null)
            {
                foreach (Delegate dl in m_OnPickItemAction.GetInvocationList())
                {
                    if (dl.Method.Equals(ac))
                    {
                        return;
                    }
                }
            ...
            m_OnPickItemAction += ac;
        }

        public void RegisterPickItem2DEvent(Action<RaycastHit2D> ac)
        {
            if (m_OnPickItem2DAction != null)
            {
                foreach (Delegate dl in m_OnPickItem2DAction.GetInvocationList())
                {
                    if (dl.Method.Equals(ac))
                    {
                        return;
            ...
            m_OnPickItem2DAction += ac;
        }

        public void UnRegisterPickItemEvent(Action<RaycastHit> ac)
        {
            foreach (Delegate dl in m_OnPickItemAction.GetInvocationList())
            {
                if (dl.Method.Equals(ac))
        ...
        public void UnRegisterPickItem2DEvent(Action<RaycastHit2D> ac)
        {
            foreach (Delegate dl in m_OnPickItem2DAction.GetInvocationList())
            {
                if (dl.Method.Equals(ac))
        ...
        private void InputControllerOnInputClick(Vector3 clickPosition, bool isDoubleClick, bool isLongTap)
        {
            ...
            if (OnPickItem != null || OnPickItemDoubleClick != null)
            {
                ...
                if (Physics.Raycast(camRay, out hitInfo) == true)
                {
                    ...
                    if (m_OnPickItemAction != null)
                    {
                        m_OnPickItemAction.Invoke(hitInfo);
                    }
                }
            }
            if (OnPickItem2D != null || OnPickItem2DDoubleClick != null)
            {
               ...
               // Debug.DrawRay(camRay.origin, 1000*camRay.direction,Color.blue,200);
                ...
                if (hitInfo2D == true)
                {
                    ...
                    if (m_OnPickItem2DAction != null)
                    {
                        m_OnPickItem2DAction.Invoke(hitInfo2D);
                    }
                }
            }
        }
