    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed SampleAlpha(int pIndex, v2f IN)
            {
                ...
                float2 uv = IN.texcoord + _MainTex_TexelSize.xy * float2(cosArray[pIndex], sinArray[pIndex]) * _OutlineWidth;
                return IsInRect(uv, IN.tangent) * (tex2D(_MainTex, uv) + _TextureSampleAdd).w * _OutlineColor.w;
            }
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                if (_OutlineWidth > 0) 
                {
                    color.w *= IsInRect(IN.texcoord, IN.tangent);//剔除uv框外的像素
					...
					//color = val * (1.0 - color.a);
					val.w *= step(color.w,0.1f);//如果不用step会出现渐变alpha时颜色错乱
					val.w=clamp(val.w,0,1);//w超出1后叠加的部分会变白
                    color = val;
                }
